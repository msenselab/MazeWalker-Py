...
classdef labJack < handle
	...
	properties (SetAccess = private, GetAccess = private)
        ...
        % added by Strongway
        AIRead0   = hex2dec(['1b'; 'f8'; '02'; '00'; '20'; '00'; '00'; '01'; '00'; '1f'])' % analogue feeedback PC-0, NC = 31
	...
	%=======================================================================
		...
		function open(obj)
			if obj.silentMode == false || isempty(obj.handle)
				...
				if obj.validHandle();
					...
					none=[0,0,0];
                    % 22.08.16, Strongway, initialize device for FIO, EIO
                    % (output), and CIO for input
                    obj.setDIODirection([255,255,0]);
					obj.setDIO(none, [255,255,0]); %Sink all our DIO to output LOW
				else
		...
		function out = rawWrite(obj,byte)
			...
			if out == 0;	
                obj.salutation('rawWrite','ERROR WRITING!',true); end
		     end
...
%				cmd(12:14) = valuedir;
        ...
        function value = getDIODirection(obj)
			if obj.silentMode == false && obj.vHandle == 1
                cmd = hex2dec(['16'; 'f8'; '01'; '00'; '1c'; '00'; '00'; '1c'])';
				obj.outp = obj.rawWrite(cmd);
                pvalue = libpointer('uint8Ptr',zeros(12,1));
                if obj.readResponse;
                    obj.inp = obj.rawRead(pvalue,12);
                end
                value = pvalue.Value(end-2:end)';
                clear pvalue
            else
                value = -1;
            end
		...
		% ===================================================================
		%> @brief getDIOValue
		%>	getDIOValue get the value for FIO, EIO and CIO as HIGH or LOW
		%>	added by Strongway, 22.08.2016
		% ===================================================================
        function value = getDIOValue(obj)
			if obj.silentMode == false && obj.vHandle == 1
				cmd=zeros(8,1);
				cmd(2) = 248; %command byte for feedback command (f8 in hex)
				cmd(3) = (length(cmd)-6)/2;
				cmd(8) = 26; %IOType for PortStateRead = 26
%                cmd = hex2dec(['14'; 'f8'; '01'; '00'; '1a'; '00'; '00'; '1a']);
				obj.command = obj.checksum(cmd,'extended');
				obj.outp = obj.rawWrite(obj.command);
                pvalue = libpointer('uint8Ptr',zeros(12,1));
                if obj.readResponse;
        ...
        % ======================================
        % send value via FIO to EEG as trigger
		%>	added by Strongway, 22.08.2016
        % ======================================
        function sendTrigger(obj, value)
            values = [value, 0, 0];
            mask = [255, 0, 0]; % only for the FIO port
            obj.setDIOValue(values, mask);
        end
        
        % ======================================
        % Get value from CIO port, which connects to response device
		%>	added by Strongway, 22.08.2016
        % ======================================
        function resp = getResponse(obj)
            value = obj.getDIOValue();
            resp = 15 - value(3);  % initial port values are high, so reverse the coding
        end
